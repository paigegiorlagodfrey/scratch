...
#!	open HD19467B and read in the arrays for wavelength, flux, and uncertainty
	file=open('/Users/paigegiorla/Code/Python/BDNYC/TDwarfplotting/HD19467B_project/HD19467NewSpect2OldWave.txt')
	W_obj,F_obj,U_obj=[],[],[]
	for line in file:
		columns=line.split()
		wav=float(columns[0])
	...
	F_obj_t=F_obj[4:30]
	U_obj_t=U_obj[4:30]
	myInt=2.6036315136513934e-17 
	F_obj_tn = [x/myInt for x in F_obj_t]													#n = normalized
	U_obj_tn = [x/myInt for x in U_obj_t]
	object=np.array([W_obj_t,F_obj_tn,U_obj_tn])
	print np.gradient(W_obj), len(W_obj_t), len(F_obj_tn), len(U_obj_tn)																#to make sure all spaces in between points are the same length
# 	plt.errorbar(object[0],object[1],yerr=object[2])
# 	plt.show()
...
#!	loop through each object
	...
	for j in idlist:
		...
		binned_wl=0
		jump=0.0286																		#defined by the wavelength points of HD19467B data
		start=1.0236-(jump/2) 															#start half way before 1st flux point
		W,F,U=u.scrub([W,F,U])
		W=W.value
		F=F.value
		U=U.value
...
#!		loop through 21 iterations for the amount of wavelength ranges I have
		...
		template=u.norm_spec(template,original)												#Joe's code to normalize plots to each other
...
#!		plot template over template's original flux
		...
		plt.savefig('/Users/paigegiorla/Code/Python/BDNYC/TDwarfplotting/HD19467B_project/NewSpectplots/binned_flux_with_original/'+'{}'.format(name)+ '.pdf')
		plt.clf()  	
...
#!		plot template over object
		...
		plt.savefig('/Users/paigegiorla/Code/Python/BDNYC/TDwarfplotting/HD19467B_project/NewSpectplots/template_over_object/'+'{}'.format(name)+ '_HD19467.pdf')
		plt.clf()
#-----------------------------------------------------------------------------------------
# 		try:
# 		print len(template[1]), len(template[2]), len(object[1]), len(object[2])
...
#!	plotting the best fits, or minimum chi squared values over the object
	...
	plt.savefig('/Users/paigegiorla/Code/Python/BDNYC/TDwarfplotting/HD19467B_project/NewSpectplots/bestfits_w_object.pdf')
 	plt.clf()	
...
#!	plot spectral type vs chi square
	...
	plt.savefig('/Users/paigegiorla/Code/Python/BDNYC/TDwarfplotting/HD19467B_project/NewSpectplots/chisquare_distribution.pdf')
	plt.clf()	
